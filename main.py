...
deg2rad = np.pi / 180
factor = 0.001
y = ziba_p * np.cos((90+ziba_ang) * deg2rad) * factor
x = ziba_p * np.sin((90+ziba_ang) * deg2rad) * factor

for i in range(len(ziba)):
    line = np.array([[ziba_coord[i].ra.deg-x[i], ziba_coord[i].ra.deg+x[i]],[ziba_coord[i].dec.deg-y[i],ziba_coord[i].dec.deg+y[i]]])
    f.show_lines([line],color='r',lw=0.4)

ra_center = 266.41
dec_center = -29.008
# f.recenter(ra_center, dec_center, width=0.1, height=0.1)
f.save('test.pdf')
# f.save('test.eps')
f.close()